...
% Importnat variables used: 
%   I - input image
%   octave - used for storing gaussian filtered image
%   sigma - standard scale factor used by Lowe
%   key - used to store key points of feature of image
%   max_mag - used for storing orientation magnitude and corresponding bin number
%   discreptor - used to store the discreptors vector of key points in the image     
      
% Author: VIJAY ANAND
%
%%
function Sift_Feature(filename, row, col)

I=imread(filename);                    % to read the image
figure;
subplot(1,2,1);
imshow(I);
...
%%      END OF SIFT ALGORITHM



% Conlusion: Descriptor Matrix contains all the descriptor vectors corresponding to each keypoint.
%            for a given key point the dimension of descriptor vector is 128.
